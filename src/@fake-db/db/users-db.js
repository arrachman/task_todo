...
mock.onGet('/api/users-app/users').reply(config => {
	const { id } = config.params;
	let response = [];
	switch (id) {
		case 'frequent': {
			response = usersDB.users.filter(contact => usersDB.user[0].frequentContacts.includes(contact.id));
			break;
		}
		case 'starred': {
			response = usersDB.users.filter(contact => usersDB.user[0].starred.includes(contact.id));
			break;
		}
		default: {
...
mock.onGet('/api/users-app/user').reply(config => {
	return [200, usersDB.user[0]];
});

mock.onPost('/api/users-app/add-contact').reply(request => {
	const { contact } = JSON.parse(request.data);
	const newContact = {
		...contact,
		id: FuseUtils.generateGUID()
	};
	usersDB.users = [...usersDB.users, newContact];
	return [200, newContact];
});

mock.onPost('/api/users-app/update-contact').reply(request => {
	const { contact } = JSON.parse(request.data);

	usersDB.users = usersDB.users.map(_contact => {
...
mock.onPost('/api/users-app/remove-contact').reply(request => {
	const { contactId } = JSON.parse(request.data);
	usersDB.users = usersDB.users.filter(contact => contactId !== contact.id);
...
mock.onPost('/api/users-app/remove-users').reply(request => {
	...
	usersDB.users = usersDB.users.filter(contact => !contactIds.includes(contact.id));
	return [200, contactIds];
});

mock.onPost('/api/users-app/toggle-starred-contact').reply(request => {
	const { contactId } = JSON.parse(request.data);
	usersDB.user[0].starred = _.xor(usersDB.user[0].starred, [contactId]);
	return [200, contactId];
});

mock.onPost('/api/users-app/toggle-starred-users').reply(request => {
	const { contactIds } = JSON.parse(request.data);
	usersDB.user[0].starred = _.xor(usersDB.user[0].starred, contactIds);
	return [200, contactIds];
});

mock.onPost('/api/users-app/set-users-starred').reply(request => {
	const { contactIds } = JSON.parse(request.data);

	usersDB.user[0].starred = [...usersDB.user[0].starred, ...contactIds];

	return [200, contactIds];
});

mock.onPost('/api/users-app/set-users-unstarred').reply(request => {
	const { contactIds } = JSON.parse(request.data);

	usersDB.user[0].starred = usersDB.user[0].starred.filter(contactId => !contactIds.includes(contactId));
